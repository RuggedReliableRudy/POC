...
resource "aws_iam_policy" "pending_updates_access" {
  name = "${local.family}-pending-updates-access"
    ...
    Statement = [
      {
        ...
        Resource = var.pending_table_arn
      }
    ]
  })
}

resource "aws_iam_role_policy_attachment" "task_pending_updates" {
  role       = aws_iam_role.task_role.name
  policy_arn = aws_iam_policy.pending_updates_access.arn
}
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "PENDING_UPDATES_TABLE", value = var.pending_table_name }
      ]
    }
  ])
